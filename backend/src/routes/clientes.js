...
router.post('/', async (req,res) => {
  try {
    ...
    const row = [
      c.id, c.nombre, c.email || '', c.telefono || '', c.nacimiento || '', c.sexo || '', c.estatura || '', c.peso || '',
      c.objetivo || '', c.nivel || '', c.dias || '', c.equipo || '', c.preferencias || '', c.alergias || '', c.patologias || '', c.historial || '', c.notas || '', c.createdAt
    ];
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // The order of fields must match the Google Sheet columns exactly
        const rowData = [
        ...
        await sheets.updateRow(id, rowData);
        res.json(c);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: err.message });
