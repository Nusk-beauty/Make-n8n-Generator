...
async function loadClientes(){
  ...
  const sel = $('#selCliente'); if (sel) { sel.innerHTML = '<option value="">— selecciona —</option>'; list.forEach(c => sel.innerHTML += `<option value="${c.id}">${c.nombre}</option>`); }
}
...
function showDetalle(c){
  ...
  $('#saveNotas').addEventListener('click', async ()=>{
    ...
    // Opcional: podrías actualizar el objeto 'c' localmente en lugar de recargar todo
    loadClientes();
  });
  $('#btnCalc').addEventListener('click', ()=> {
    ...
    const cObj = { sexo: c.sexo || 'Masculino', cintura: Number(cintura), cuello: Number(cuello), altura: Number(altura), cadera: 0 };
    if (cObj.sexo === 'Femenino') cObj.cadera = Number(prompt('Medida cadera (cm)'));
    fetch(`${API_BASE}/api/extra/calcFat`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(cObj) })
...
$('#btnGenerarPlan')?.addEventListener('click', async ()=>{
  ...
  const gen = await fetch(`${API_BASE}/api/planes/${clienteId}`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ cliente, dias, enfoque, enviarEmail:false }) });
...
$('#btnGenerarPdf')?.addEventListener('click', async ()=>{
  ...
  const res = await fetch(`${API_BASE}/api/report/send`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ cliente: window.lastPlan.cliente, plan: window.lastPlan.plan }) });
...
$('#chatForm')?.addEventListener('submit', async (e)=>{
  ...
  const r = await fetch(`${API_BASE}/api/gemini`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ prompt: q }) });
